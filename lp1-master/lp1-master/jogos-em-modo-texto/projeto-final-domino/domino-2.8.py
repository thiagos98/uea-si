...
def verificar_jogada(jogada,peca_restante, pontas):
    cont = 0
...
def atualizar_ponta(jogada,pontas):
...
def atualizar_pecas_restante(jogada,peca_restante):
    for i in range(len(peca_restante)-1):
...
while len(peca)>0:
    print("Exemplo: 6,5 \n")
    jogada = capturar_peca()                                      #Captura peca jogada
    ...
    if ponta1 == ponta2:
        ...
        carroca = False
    verifica = verificar_jogada(jogada,peca_restante,pontas)      #Verifica Jogada Valida
    if verifica == True:                                         #Se Jogada Valida Atualiza pontas e pontos
        pontas,ponta,verifica = atualizar_ponta(jogada,pontas)
        if verifica == True:
            peca_restante = atualizar_pecas_restante(jogada, peca_restante)
